...
document.addEventListener('puzzleInputLoaded', () => {
      ...
      for (const ring1 of Object.keys(shop.Rings)) {
        for (const ring2 of Object.keys(shop.Rings)) {
          if (ring1 !== ring2) { // Ensure rings are not duplicated
            ...
            const winner = battle()
            if (winner === 'player') perBattleCost.push(totalCost)
  ...
  document.getElementById('answer').innerText = Math.min(...perBattleCost)
  console.log('The least amount of gold for a win is', Math.min(...perBattleCost))
})
