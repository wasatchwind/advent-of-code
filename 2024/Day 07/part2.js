...
document.addEventListener('puzzleInputLoaded', () => {
  ...
  // Day 7 Part 2 answer: 189207836795655
  ...
  function validateEquation(equation) {
    const operators = ['+', '*', '|'] // '|' added for Part 2
    ...
    function evaluate(combos) {
      for (const combo of combos) {
        const tokens = combo.split(/([+*|])/) // '|' added for Part 2
        ...
        for (let index = 1; index < tokens.length; index += 2) {
          ...
          if (tokens[index] === '*') calculationResult *= nextNumber
          if (tokens[index] === '|') calculationResult = Number(calculationResult.toString() + nextNumber.toString())
        }
